...
from ee_auth import init_earth_engine

st.set_page_config(
    page_title="Satellite Timelapse",
    page_icon=":satellite:",
    layout="wide",
)
...
st.sidebar.info(
    """
    Create satellite timelapse animations (Landsat, Sentinel-2, GOES, MODIS, NAIP)
    for any region on Earth. Built for agricultural and environmental research.

    Forked from [opengeos/streamlit-geospatial](https://github.com/opengeos/streamlit-geospatial).
    See `README.md` for setup and credential handoff.
    """
)
...
def app():
    ...
    st.markdown(
        """
        Generate timelapse animations of [Landsat](https://developers.google.com/earth-engine/datasets/catalog/landsat),
        Sentinel-2, [GOES](https://jstnbraaten.medium.com/goes-in-earth-engine-53fbc8783c16), MODIS, or NAIP imagery
        for any region on Earth. Built with [Streamlit](https://streamlit.io), [geemap](https://geemap.org), and
...
try:
    ...
    st.info(
        "If this is an authentication error, double-check the "
        "`GCP_SERVICE_ACCOUNT_JSON` and `EE_PROJECT_ID` secrets. "
